...
class App extends Component {
  ...
  uploadFile = description => {
    ...
    ipfs.add(this.state.buffer, (error, result) => {
      ...
      this.state.storage.methods.uploadFile(result[0].hash, result[0].size, this.state.name, this.state.type, description).send({ from: this.state.account }).on('transactionHash', (hash) => {
        this.setState({
          loading: false,
          type: null,
          name: null
